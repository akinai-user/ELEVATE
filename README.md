...
フリーランスエンジニアの案件紹介・面談登録は、既存の外部LP（`https://elevate-works.jp/lp/`）へ案内します。

合計22ページ。別途、旧サービス6URLは移転ページとして維持しています。
...
- TOPには仮プロフィールによる社員インタビューを掲載しています。横スワイプ・前後ボタン・位置ボタンで切り替え、各カードから詳細モーダルを開けます。詳細は `content/pages/index.html` の `interview-story-01`〜`03` に記載。人物名・肩書き・本文・写真は、公開前に実データへ差し替えてください。
- 会社情報の重複は会社ページへ集約。数値は従来の会社ページを継承。古いLP側の異なる数値は削除。
- フリーランスエンジニア向けの導線は、既存の外部LPへ別ウィンドウで遷移します。
